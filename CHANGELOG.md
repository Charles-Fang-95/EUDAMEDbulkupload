...
- 依据 EUDAMED Production 2.15.0 §3.2 和 Playground 3.9 §3.2，移除 Orthopedic/Orthopaedic 特殊类型的模板选项。XSD 仍保留旧枚举，但不等于当前平台允许注册。
- 模板升级 v2.14，中英文说明同步；保留多行模板容量、现有数据和旧模板兼容性。
- 导入、模板迁移及已有数据库记录的导出预检识别骨科旧值（含 MDR/MDD/AIMDD 前缀及两种拼写），给出明确提示；禁止含该旧值的 Basic 注册/更新导出，不静默清空或改为其他类型。
- 保留软件和其余有效特殊类型；不更改官方 XSD 文件和器械风险分类规则。

## 1.0.3 - 2026-09-15

- 导入采用流式读取，只保留非空值及其数据类型、数字格式，减少模板预设空白单元格的内存构建；原文件不变。
- 导入提交后显示等待提示并阻止重复提交；完成后定位到结果区，保留本批次产品库和导出入口。
- 导入结果和导出预检按红色错误、黄色警告、绿色无问题区分，并显示数量，结果不会自动消失。

## 1.0.2 - 2026-09-15

- 修复检查更新及国内下载入口错误显示 Gitee 0.7 系列：改用 Gitee latest 接口；列表回退按版本号选择，不再假定首项最新。

## 1.0.1 - 2026-09-15

- Mac 主动取消保存不再误报“下载失败”，真正的下载错误仍显示提示。

- 修复 sessionStorage 筛选恢复逻辑在 POST 返回后重定向并丢失导出/预检结果的问题。
- 修复导出成功后不触发下载的问题：生成 XML/ZIP 后自动请求下载，结果及手动下载入口置于产品列表前；预检和校验失败不会触发下载。取消保存后可直接使用结果页按钮重试。
- 修复同一秒重复导出可能覆盖历史 XML/ZIP 的问题：每次导出使用独立随机文件标识，历史下载保持对应原文件。
- 导出任务在仅导出勾选记录且未选择产品时提示用户，避免空选提交。
...
- Mac 更新为同版 0.9.9：打包当前中英文模板，补齐文件选择和下载保存，持续读取服务日志，使用独立端口启动随包后端，避免连接旧版本服务；仍需本机 Python 3（建议 3.10+）。构建输出本机架构 ZIP，使用本地 ad-hoc 签名，未作 Apple 公证。
- Windows 构建补齐 openpyxl/et_xmlfile 构建依赖，检查编译和 PyInstaller 退出码，避免失败后打包旧产物。两平台缩小资源范围为运行所需 Python 模块、模板和生产 XSD，不携带完整审计目录或客户资料。
