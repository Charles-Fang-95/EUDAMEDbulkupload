...
def _how_to_use_lines_zh():
    lines = [
        ...
        ("如旧记录已有 EUDAMED 分配的 B-/D- 标识，可不填计算主体和 Local - Record ID，但必须同时提供合法且后缀完全一致的 B-/D- 对；关联 sheet 应直接填写正式 B-/D- 编码。导入后可下载不覆盖原文件的规范化 v2.14 模板副本。", None),
