...
def _how_to_use_lines_zh():
    lines = [
        ...
        ("如旧记录已有 EUDAMED 分配的 B-/D- 标识，可不填计算主体和 Local - Record ID，但必须同时提供合法且后缀完全一致的 B-/D- 对；关联 sheet 应直接填写正式 B-/D- 编码。导入后可下载不覆盖原文件的规范化 v2.13 模板副本。", None),
        ...
        ("Original manufacturer：Product Designer SRN 与组织名称/地址二选一；支持注册上传和 PRODUCT_DESIGNER.PUT。地址填写时国家和邮编必填，旧 Product Designer ID 请留空。暂不提供联系人或删除操作。", None),
        ("URL for additional information 会输出到官方 DTX XML 的 udidi:website；可填写产品信息页或 eIFU 网页入口。Public Email、Basic - Presence of Medicinal Substance 等字段仍在官方字段映射审计中。", None),
...
def _how_to_use_lines_en():
    ...
    return [
        ...
        ("Original manufacturer: choose Product Designer SRN or organisation name/address. Supports registration and PRODUCT_DESIGNER.PUT. Country and postal code are required with an address. Leave the deprecated Product Designer ID blank. Contacts and deletion are not supported.", None),
        ("URL for additional information is exported to the official DTX XML element udidi:website and may point to a product information page or eIFU webpage. Public Email and Basic - Presence of Medicinal Substance remain under official field-mapping review.", None),
