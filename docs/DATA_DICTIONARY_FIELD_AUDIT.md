...
| DD BASIC UDI | FLD-UDID-60 | Certificate Type (Technical Documentation, Type Examination, etc) | 1 Occurrence applicable if Device Certificate Inforamtion is provided (FLD-UDID-39) | Device Certificates | Yes | Payload | Yes | basicudi:deviceCertificateLinks/links:deviceCertificateLink | `implemented` | Structured certificate rows are exported as deviceCertificateLinks. PR/SPP certificate handling remains out of scope. |
...
| DD BASIC UDI | FLD-UDID-357 | Decision Date | 0..1 Occurrence applicable if Certificate Link exists (FLD-UDID-343) |  | No | No | No |  | `not_in_template` | Certificate decision metadata is not part of current deviceCertificateLinks output. |
